	...
	<div class="sou">
        ...
        <span class="fr">
        	<c:if test="${sessionScope.loginUser == null }">
        		<span class="fl">你好，请<a href="${ctx }/Login?action=toLogin">登录</a>&nbsp;
        		 <a href="${ctx }/Register?action=toRegister" style="color:#ff4e00;">免费注册</a>&nbsp;|
        		</span>
        	</c:if>
        	<c:if test="${sessionScope.loginUser != null }">
        		<span class="fl">&nbsp;
        			<a href="${ctx }/Login?action=toLogin">${sessionScope.loginUser.userName }</a>&nbsp;|
        		</span>
        	</c:if>
        	<c:if test="${sessionScope.loginUser != null && sessionScope.loginUser.type == 1}">
        		<span class="fl">&nbsp;
        			<a href="${ctx }/Login?action=toLogin">后台管理</a>&nbsp;| 
        		</span>
        	</c:if>
        	<c:if test="${sessionScope.loginUser != null}">
        		<span class="fl">&nbsp;
